/**
 * permission-arity.ts — bash command-prefix arity for permission rules.
 *
 * Port of opencode's `permission/arity.ts`. The ARITY map says, for each
 * supported command, "how many tokens define the canonical, human-
 * understandable shape of this command". `prefix(tokens)` picks the
 * longest matching ARITY entry from `tokens`, then returns that many
 * tokens.
 *
 * Examples (from the openclaw/opencode rule set):
 *
 *   ["git", "push", "origin", "main"]    → ["git", "push"]   (git: 2)
 *   ["npm", "run", "dev"]                → ["npm", "run", "dev"] (npm run: 3)
 *   ["docker", "compose", "up", "-d"]    → ["docker","compose","up"] (docker compose: 3)
 *   ["python", "script.py"]              → ["python", "script.py"] (python: 2)
 *
 * Used by permission-preview.ts to convert a one-time approval into a
 * canonical rule pattern (`<prefix> *`) so future calls with the same
 * canonical shape are auto-approved without re-prompting.
 *
 * The ARITY dictionary itself is large (~140 entries) and was
 * LLM-generated by opencode. We port verbatim — the cost of a missing
 * entry is just a noisier prompt, not a correctness issue, and the
 * upstream list covers stacks the user is likely to hit.
 */

/**
 * Strip flag tokens (anything starting with `-`) from a token list. Flag
 * VALUES (e.g. the `/path` after `-C`) are kept — distinguishing flags
 * that consume args from those that don't would require a per-command
 * grammar that the upstream arity table doesn't provide either. The
 * downstream `prefix` matching is robust to a few extra non-flag tokens
 * because it uses longest-prefix, so the worst case is one wider rule
 * pattern, not an incorrect one.
 */
export function stripFlags(tokens: string[]): string[] {
  return tokens.filter((t) => !t.startsWith('-'));
}

/**
 * Pick the canonical prefix tokens for a bash invocation. Tries the
 * longest joined prefix that exists in ARITY; if none match, falls back
 * to the first token (or `[]` for empty input). Mirrors opencode's
 * `permission/arity.ts:prefix` exactly — same algorithm, same order.
 */
export function prefix(tokens: string[]): string[] {
  for (let len = tokens.length; len > 0; len--) {
    const joined = tokens.slice(0, len).join(' ');
    const arity = ARITY[joined];
...
/**
 * Convenience: tokenize a raw command string, strip flags, and return
 * the canonical prefix joined as a string. The shape callers most
 * commonly want.
 */
export function canonicalPrefix(command: string): string {
...
/**
 * Command-prefix arity dictionary. Generated by opencode via LLM with
 * the prompt preserved in the original source. Keys are canonical
 * prefix strings; values are how many tokens of the input belong to
 * the "canonical command name".
 *
 * Rules of construction (from the source comment):
 *   1. Flags NEVER count as tokens — they're filtered out before this
 *      table is consulted.
 *   2. Longest matching prefix wins.
 *   3. Only include longer prefixes when the arity differs from the
 *      shorter one (e.g. `git: 2` covers `git checkout`, but `git stash`
 *      lists `git stash: 3` because it takes a sub-subcommand).
 *
 * Ported verbatim from
 * `~/develop/opencode/packages/opencode/src/permission/arity.ts:24-161`.
 */
